...
// Search for users by name, excluding the logged-in user and their friends
const searchUser = TryCatch(async (req, res) => {
  const { name = "" } = req.query;

  // Finding all chats where the logged-in user is a member
  const myChats = await Chat.find({ groupChat: false, members: req.user });

  // Extracting all users from the chats, excluding the logged-in user
...
// Get notifications (friend requests) for the logged-in user
...
const getMyFriends = TryCatch(async (req, res, next) => {
  try {
    ...
    const friends = chats.map(({ members }) => {
      ...
      if (!otherUser) {
        console.error("Other member not found in chat members");
        return null;
      }
